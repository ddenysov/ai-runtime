...
Рекомендуемый путь интеграции: добавить пакет `neuron-core/a2a`, создать A2A server через Artisan, реализовать task repository на PostgreSQL, запускать выполнение через Laravel Queue, а результат и промежуточные статусы доставлять через A2A push notifications, если клиент передал webhook configuration. Neuron agents лучше создавать через фабрику по `slug`: на первом этапе slug ищется в Laravel config с несколькими дефолтными агентами, а позже тот же контракт можно переключить на настройки из БД.
...
### Шаг 5. Реализовать фабрику Neuron agents
...
3. Tool отправляет `message/send` или `SendMessage` удаленному A2A agent.
...
Для durable resume нужны отдельные таблицы. Названия можно уточнить при реализации, но модель должна хранить parent run, tool calls и связь с remote A2A task.
...
`conversation_state` и `workflow_state` зависят от того, как именно Neuron будет сериализовать текущий run. Если Neuron workflow не дает готового сериализуемого continuation, нужно хранить достаточную историю сообщений и pending tool call, чтобы новая job могла реконструировать следующий шаг агента.
...
Вместо реального exception можно использовать interrupt/pause механизм Neuron, если он подходит для сериализации состояния. Главное требование: job должна завершиться штатно, а не висеть в ожидании remote result.
...
4. Вставить результат как tool result в формат, который понимает Neuron Agent.
5. Перевести run в `running`.
6. Запустить parent agent дальше.
...
Durable resume. Parent agent нельзя держать в памяти между remote A2A request и webhook result. Решение: сохранять `agent_runs`, `agent_tool_calls`, `a2a_child_tasks` и продолжать выполнение только через новую queue job.
...
11. Сделать `message/send` queue-first: вернуть `SUBMITTED`, dispatch `ProcessA2ATask`.
...
18. Добавить feature tests для Agent Card, auth, async `message/send`, agent factory lookup по slug, queue job, webhook delivery и parent resume.
19. После MVP решить, нужна ли поддержка A2A 1.0 `SendMessage` или достаточно формата, который предоставляет текущий PHP пакет.
