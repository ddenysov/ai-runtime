...
12. Добавить `RemoteA2AAgentTool`, который создает child A2A task и переводит parent run в `waiting_for_tool`.
13. Добавить таблицы `agent_runs`, `agent_tool_calls`, `a2a_child_tasks` для durable pause/resume.
14. Добавить webhook для remote tool results и `ResumeParentAgentJob`.
15. Закрыть endpoint Bearer-token middleware.
16. Добавить feature tests для Agent Card, auth, async `message/send`, queue job, webhook delivery и parent resume.
17. После MVP решить, нужна ли поддержка A2A 1.0 `SendMessage` или достаточно формата, который предоставляет текущий PHP пакет.
