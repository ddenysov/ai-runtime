...
final class McpStdioToolExecutor
{
    ...
    public function execute(McpServer $server, string $toolName, array $arguments): string
    {
        ...
        if (! $resolved['ok']) {
            Log::channel(self::MCP_AGENT_LOG_CHANNEL)->debug('MCP agent tool response blocked', [
        ...
        $client = Client::builder()
            ->setClientInfo('AI Runtime Agent MCP', '1.0.0')
            ->setInitTimeout(30)
        ...
        try {
            ...
            Log::channel(self::MCP_AGENT_LOG_CHANNEL)->debug('MCP agent tool response failed', [
                'mcp_server_uuid' => $server->uuid,
                'mcp_server_name' => $server->name,
                'tool' => $toolName,
                'arguments' => $arguments,
